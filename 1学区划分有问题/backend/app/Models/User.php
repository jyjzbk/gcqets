...
class User extends Authenticatable
{
    ...
    {
        // 获取用户直接关联的组织
        $directOrganizations = $this->organizations;
        ...
        if ($primaryOrg) {
            ...
            // 如果用户有管理权限，还可以访问下级组织
            if ($this->hasPermission('organization.manage')) {
                $descendants = $primaryOrg->getDescendants();
                $organizations = $organizations->merge($descendants);
            }
        }

        $organizations = $organizations->merge($directOrganizations);

        return $organizations->unique('id');
    }
